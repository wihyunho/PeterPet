...
public class SocialController {
	...
	public String naverRegister(HttpServletRequest request) {
		if (udao.loginCheck(request)) {
			request.getSession().setAttribute("messageType", "오류 메시지");
			request.getSession().setAttribute("messageContent", "로그인 상태로 회원가입을 할 수 없습니다.");
			request.setAttribute("contentPage", "home.jsp");
			...
			if(udao.getUser2(request.getParameter("naver_ID"))==null){
				request.setAttribute("contentPage", "account/naver_join.jsp");
				request.setAttribute("userID",  request.getParameter("naver_ID"));
				request.setAttribute("userPassword",  request.getParameter("naver_Password"));
				request.setAttribute("userName",  request.getParameter("naver_Name"));
				request.setAttribute("profile",  request.getParameter("naver_profile"));
	...
	// 네이버 회원 가입
	@RequestMapping(value = "/naverRegister2", method = RequestMethod.POST)
	public String naverRegister2(HttpServletRequest request) {
		String userID = request.getParameter("userID");
